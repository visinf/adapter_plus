...
# modified from https://github.com/KMnP/vpt/blob/main/src/data/datasets/tf_dataset.py
import functools
...
from data.constants import NUM_CLASSES_VTAB
...
DATASETS = {
    ...
    "eurosat": 'eurosat',
    "dmlab": 'dmlab',
    "kitti": 'kitti(task="closest_vehicle_distance")',
    "smallnorb_azimuth": 'smallnorb(predicted_attribute="label_azimuth")',
    "smallnorb_elevation": 'smallnorb(predicted_attribute="label_elevation")',
    "dsprites_position": 'dsprites(predicted_attribute="label_x_position",num_classes=16)',
    "dsprites_orientation": 'dsprites(predicted_attribute="label_orientation",num_classes=16)',
    "clevr_distance": 'clevr(task="closest_object_distance")',
    "clevr_count": 'clevr(task="count_all")',
    "retinopathy": 'diabetic_retinopathy(config="btgraham-300")'
}


class TFDataset(torch.utils.data.Dataset):
    def __init__(self, dataset, data_dir, split, mean, std):
        assert split in {
            "train",
            "val",
            "test",
        ...
        self.get_data(dataset, data_dir, split)

    def get_data(self, dataset, data_dir, split):
        tf_data = build_tf_dataset(dataset, data_dir, split)
    ...
    def get_info(self):
        ...
        # num_imgs = len(self._data_list)
        return num_imgs, self.get_class_num()

    def get_class_num(self):
        return NUM_CLASSES_VTAB[self.name]
    ...
    def get_class_weights(self, weight_type):
        ...
        # disabled because _class_ids is not populated
...
def build_tf_dataset(dataset, data_dir, split, crop_size=224):
    """
    ...
    if split not in ["train", "val", "test", "trainval"]:
        raise ValueError("The input pipeline supports `train`, `val`, `test`."
                         "Provided mode is {}".format(split))
    is_train = split in ["train", "trainval"]

    vtab_dataname = dataset
    if vtab_dataname in DATASETS.keys():
        data_cls = Registry.lookup("data." + DATASETS[vtab_dataname])
    ...
    return vtab_tf_dataloader.get_tf_data(
        ...
        split_name=split_name_dict[f"dataset_{split}_split_name"],
        preprocess_fn=functools.partial(
            preprocess_fn,
            input_range=(0.0, 1.0),
            size=crop_size,
            ),
        for_eval=(not is_train),  # handles shuffling
...
def to_torch_imgs(img: np.ndarray, mean: Tensor, std: Tensor) -> Tensor:
    t_img: Tensor = torch.from_numpy(np.transpose(img, (2, 0, 1)))  # H, W, C -> C, H, W
    t_img -= mean
    t_img /= std
